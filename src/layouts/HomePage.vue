  ...
  <q-layout view="hHh LpR fFf">
      ...
      <q-toolbar>
        ...
        <q-toolbar-title class="title-main" width="70%">
          <div class="row items-center text-h6 text-bold" style="gap: 10px">
            ...
            <span class="text-italic"> CISC-UNL</span>
          </div>
        </q-toolbar-title>
        ...
        <div class="notificaciones">
          <q-btn dense flat round icon="mdi-bell">
            <q-badge size="xs" floating color="red" v-if="notificacionNoLeidas > 0">{{
              notificacionNoLeidas
            }}</q-badge>
            <q-menu class="q-pa-none" @show="notificaionesAbiertas">
              <q-item-label
                header
                class="text-h6 q-py-sm q-ma-none text-bold text-primary"
              ...
              <q-list
                class="scroll"
                dense
                style="min-width: 250px; max-width: 350px; max-height: 50vh"
              >
                ...
                <template v-else v-for="item of notificaciones" :key="item">
                  <q-item>
                    <q-item-section>
                      <q-item-label
                        :class="
                          'text-caption text-justify ' + (item.leido ? 'text-grey-7' : '')
                        "
                      >
                        <b>{{
                          item.origen.primerNombre +
                          " " +
                          item.origen.primerApellido +
                          " "
                        }}</b>
                        {{ item.mensaje }}
                      </q-item-label>
                    ...
                    <q-item-section side>
                      <q-item-label
                        caption
                        :class="!item.leido ? 'text-light-blue-9 text-bold' : ''"
              ...
              <div class="flex text-center justify-center q-pa-sm">
                <q-btn
                  ...
                  color="primary"
                  class="q-px-md"
                  @click="obtenerTodasNotificaciones"
                  >Ver todas</q-btn
                >
              </div>
            </q-menu>
    ...
    <q-drawer show-if-above v-model="leftDrawerOpen" side="left" bordered :width="315">
      <q-img style="height: 150px" src="~assets/font-user.jpg" class="font-img">
        <q-card-section
          style="z-index: 1"
          class="bg-transparent absolute-bottom text-subtitle2 text-left"
        >
          <q-avatar class="items-end q-mb-sm">
            <img src="~assets/user-none.png" />
          </q-avatar>
          <div class="text-h7 text-subtitle1 text-bold">
            {{
              userStore.user
                ? userStore.user.primerNombre + " " + userStore.user.primerApellido
                : ""
            }}
          </div>
          <div class="text-subtitle2 text-italic">
            {{
              userStore.user ? userStore.user.usuario.rol.nombre.toUpperCase() : "DOCENTE"
            }}
          </div>
        </q-card-section>
      </q-img>
      ...
      <q-list
        bordered
        v-if="
          userStore.user && userStore.user.usuario.rol.nombre.toUpperCase() == 'DIRECTOR'
        "
      >
        <q-item
          v-for="link in linksDirector"
          :key="link.name"
          active-class="text-blue-9"
          :to="link.to"
          exact
        >
          ...
          <q-card-section class="q-col-gutter-md">
            ...
            <q-input
              ...
              :rules="[
                ...
                (val) =>
                  (val.length > 7 && val.length < 21) ||
                  'Solo se permite una contraseña entre 8 y 20 caracteres',
              ]"
            ...
            <template v-else v-for="item of todasNotificaciones" :key="item">
              <q-item>
                <q-item-section>
                  ...
                  >
                    <b>{{
                      item.origen.primerNombre + " " + item.origen.primerApellido + " "
                    }}</b>
                    {{ item.mensaje }}
                  </q-item-label>
                </q-item-section>
        ...
        <q-form @submit="cambiarDedicacion()" class="q-gutter-x-md">
          <q-card-section class="q-mb-sm">
            <q-input
              ...
              outlined
              square
              v-model="editDedicacion"
              label="Dedicación Actual"
              lazy-rules
              :rules="[(val) => (val && val.length > 0) || 'Complete este campo']"
              hint="Ingrese su dedicación actual"
              type="text"
            />
          </q-card-section>
          <q-separator />
...
const linksDirector = [
  ...
  {
    title: "Informes de los Docentes",
    icon: "mdi-file-check",
    requireDirector: true,
    to: "/informe/revision",
  },
...
function notificaionesAbiertas() {
  ...
  for (let i = 0; i < notificaciones.value.length; i++) {
    notificaciones.value[i].tiempoTranscurrido = tiempoTranscurrido(
      notificaciones.value[i].created_at
    );
...
async function logout() {
  if (Cookies.get("auth-informefinal"))
    Cookies.remove("auth-informefinal", { path: "/" });
