  ...
  <q-card class="q-ma-md" square flat bordered>
    ...
    <q-card-section class="no-padding">
      <q-table
        ...
        row-key="name"
        separator="cell"
        :no-data-label="'Aún no existen informe generados'"
      ...
      >
          ...
          <div class="row justify-between items-center full-width">
            <div>
              <div class="text-subtitle1">
                ...
                >{{ periodoSelected != null ? periodoSelected.nombre : "" }}
              </div>
              <span class="text-subtitle1 text-bold">ESTADO: </span
              ><q-chip
                v-if="periodoSelected != null"
                size="sm"
                square
                :color="periodoSelected.estado ? 'green-5' : 'red-5'"
                text-color="white"
        ...
        <template v-slot:body-cell-estadoInforme="props">
          <q-td :props="props">
            <q-chip
              size="sm"
              square
              :color="props.row.estadoInforme.color"
              text-color="white"
            ...
            <template v-if="props.row.firma_director == null">
              ...
              <q-btn round flat color="grey" size="xs" icon="mdi-help-circle-outline">
                <q-tooltip max-width="150px" style="text-align: center"
                  >Aprobación pendiente</q-tooltip
                >
              </q-btn>
            </template>
            <template v-else>
        ...
        <template v-slot:body-cell-firma_docente="props">
          <q-td :props="props">
            <template v-if="props.row.firma_docente == null">
              ...
              <q-btn round flat color="grey" size="xs" icon="mdi-help-circle-outline">
                <q-tooltip max-width="150px" style="text-align: center"
                  >El docente aun no ha enviado el informe</q-tooltip
                >
              </q-btn>
            </template>
            <q-btn
        ...
        <template v-slot:body-cell-acciones="props">
          <q-td
            ...
            auto-width
            v-if="
              periodoSelected.estado &&
              props.row.firma_docente != null &&
              props.row.estadoInforme.estado != 'Aprobado'
            "
          >
            <div>
              <q-btn
                @click="mostrarModalEnviarInforme(props.row)"
                size="xs"
                color="pink-7"
              ...
              <q-btn round flat color="grey" size="xs" icon="mdi-help-circle-outline">
                <q-tooltip max-width="150px" style="text-align: center"
                  >Enviar informe final aprobado</q-tooltip
                >
              </q-btn>
            </div>
            <div
              v-if="
                props.row.estadoInforme.estado != 'Novedad documento' &&
                props.row.idDocente != userStore.user._id
              "
            >
                ...
                >
                  <q-item
                    ...
                    @click="
                      enviarNotificacion(
                        props.row,
                        'indica que el documento del informe final enviado no es correcto'
                      )
                    "
                  >
                    <q-item-section>
                  ...
                  <q-item
                    ...
                    @click="
                      enviarNotificacion(
                        props.row,
                        'indica que la firma del informe enviado final no es válida'
                      )
                    "
                  >
                    <q-item-section>
                  ...
                  <q-item
                    ...
                    @click="
                      enviarNotificacion(
                        props.row,
                        'indica que el informe final enviado no contiene la firma del docente'
                      )
                    "
                  >
                    <q-item-section>
              ...
              <q-btn round flat color="grey" size="xs" icon="mdi-help-circle-outline">
                <q-tooltip max-width="150px" style="text-align: center"
                  >Notificar al Docente sobre problemas encontrados en el documento que ha
                  enviado</q-tooltip
                >
              </q-btn>
            </div>
          ...
          <q-td v-else auto-width>
            ...
            <q-btn round flat color="grey" size="xs" icon="mdi-help-circle-outline">
              <q-tooltip max-width="100px" style="text-align: center">
                <span v-if="!periodoSelected.estado"> Este periodo ya ha culminado </span>
                <span v-else-if="props.row.firma_docente == null">
                  El docente aun no ha enviado el informe
                </span>
                <span v-else-if="props.row.firma_director == null">
                  Aprobación pendiente
                </span>
                <span v-else> El documento ha sido aprobado </span>
              </q-tooltip>
            </q-btn>
          </q-td>
        </template>
                    ...
                    <q-item-label v-else-if="col.name == 'acciones'">
                      ...
                      >
                        ...
                        <div v-if="props.row.estadoInforme.estado != 'Novedad documento'">
                              ...
                              <q-card style="max-width: 400px; min-width: 300px">
                                ...
                                <q-card-actions class="text-center justify-end">
                                  ...
                                  <q-btn
                                    :disabled="mensajePersonalizado == ''"
                                    @click="
                                      enviarNotificacion(props.row, mensajePersonalizado)
                                    "
                                    color="positive"
                                    >Enviar</q-btn
                                  >
                          ...
                          >
                            <q-tooltip max-width="150px" style="text-align: center"
                              >Notificar al Docente sobre problemas encontrados en el
                              documento que ha enviado</q-tooltip
                            >
                          </q-btn>
                        </div>
                      </q-td>
                      ...
                      <q-td v-else auto-width>
                        ...
                        >
                          <q-tooltip max-width="100px" style="text-align: center">
                            ...
                            <span v-else-if="props.row.firma_director == null">
                              Informe pendiente de firma por parte del Director
                            </span>
                            <span v-else> El documento ha sido aprobado </span>
                          </q-tooltip>
                        </q-btn>
          ...
          <q-card-section>
            <div class="row items-center">
              ...
              <div class="col-9 col-sm-10 q-pt-xs text-justify">
                <span
                  >Usted ya ha enviado el informe final firmado al docente
                  <b>{{ docenteAEnviarInforme.nombre }}</b>
                  ¿Está seguro que desea reemplazar el informe?
                </span>
              </div>
            </div>
          </q-card-section>
        ...
        <q-card style="max-width: 400px">
          <q-card-section>
            <div class="row items-center">
              ...
              <div class="col-9 col-sm-10 q-pt-xs text-justify">
                <span
                  >Se ha notificado al Docente sobre la aprobación y envío del informe
                  final.
                </span>
              </div>
            </div>
    ...
    <q-card style="max-width: 400px">
      <q-card-section>
        <div class="row items-center">
          ...
          <div class="col-9 col-sm-10 q-pt-xs text-justify">
            <span>Se ha enviado la notificación al Docente con éxito. </span>
          </div>
        </div>
      </q-card-section>
      <q-card-actions style="display: flex; justify-content: flex-end">
  ...
  await informeController.obtenerTodosPorPeriodo(periodoSearch._id, (res) => {
    ...
    for (let i = 0; i < informes.length; i++) {
      dataInformes.value.push({
        ...
        docente:
          informes[i].docente.primerNombre + " " + informes[i].docente.primerApellido,
        estadoInforme: devolverEstado(informes[i].estado),
        firma_docente: informes[i].documento_firma_docente,
        firma_director:
          informes[i].documento_firma_director != null
            ? informes[i].documento_firma_director
            : null,
        periodo: informes[i].periodoAcademico.nombre,
        fechaInicio: new Date(
          informes[i].periodoAcademico.fechaInicio
        ).toLocaleDateString(),
...
function devolverEstado(estado) {
  ...
  if (estado == "enviadoFirmar")
    return { estado: "Aprobación pendiente", color: "orange-5" };
  if (estado == "novedadDocumento")
    return { estado: "Novedad documento", color: "red-5" };
...
async function enviarInforme() {
  ...
  await informeController.guardarInforme(formData, async (res) => {
    ...
    if (docenteAEnviarInforme.value.id == userStore.user._id)
...
async function enviarNotificacion(informe, notificacion) {
  ...
  await notificacionController.guardarNotificacion(data, (res) => {
    if (res.status != 200)
      return generateMessage("NO OK", "Ocurrió un error al notificar al director");
