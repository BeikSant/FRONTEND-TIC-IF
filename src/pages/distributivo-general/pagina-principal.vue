    ...
    <q-card-section class="no-padding no-margin">
      <transition appear enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
        <div v-show="isData" class="no-padding no-margin">
          ...
          <template v-else>
            <q-tabs indicator-color="white" v-model="funSustantivaModel" dense class="bg-primary text-grey-6" inline-label
              outside-arrows mobile-arrows active-color="white">
            ...
            <q-tab-panels animated v-model="funSustantivaModel">
              <q-tab-panel class="no-padding" v-for="(fs, index) in funcionesSustantivas" :key="index" :label=fs.nombre
                :name=fs.nombre>
                <div class="no-padding">
                  <q-table flat bordered square :rows-per-page-options="[5, 10, 0]" :pagination="{ rowsPerPage: 10 }"
                    rows-per-page-label="Resultados por página" :rows=fs.actividadesDistributivo :columns=columns
                    row-key="name" separator="cell" dense :grid="$q.screen.xs" no-results-label="No hay ningún resultado"
                    :filter="filter">
                    <template v-slot:top>
                      <div class="row justify-between items-center full-width">
                        <span class="text-h6 text-bold">LISTA DE ACTIVIDADES</span>
  ...
  <q-dialog v-model="fixed" square persistent>
    <q-card>
      <q-card-section class="bg-primary row justify-between">
        ...
        <q-btn target="_blank" :href="baseURL + '/documents/Formatodistributivo.xlsx'" color="info" size="sm">
          Ver Formato</q-btn>
      </q-card-section>

      <q-card-section class="scroll q-gutter-y-sm" v-if="actividadesSubir.length != 0" style="max-height: 50vh">
        <template v-for="fs of actividadesSubir" :key="fs">
          <q-toolbar style="min-height: 35px;" class="bg-primary  text-weight-bolder text-white q-py-xs">
      ...
      <q-card-section v-if="actividadesSubir.length == 0">
        <q-banner class="bg-indigo-1 text-justify">
          ...
          <span class="text-caption">
            Para cargar un nuevo distributivo general de los docentes, debe regirse al formato del documento .xlsx que se
            puede
            obtener al dar click
            en <b>VER FORMATO</b>. <br />La <b>CODIFICACIÓN</b> de las actividades debe ser igual al de la
            planificación de carga horaria docente del <b>SIAAF</b> (por ejemplo: '<b>AD 1</b>').</span>
        </q-banner>
      </q-card-section>

      <q-card-section class="q-pt-none" v-if="actividadesSubir.length == 0">
        <q-file class="full-width" outlined square bottom-slots accept=".xlsx" use-chips v-model="excelUpload"
          label="Documento .xlsx" max-file-size="2000000" counter
          hint="Suba el documento que contiene el distributivo general">
          <template v-slot:prepend>
            <q-icon name="cloud_upload" @click.stop.prevent />
          </template>
          <template v-slot:append>
            <q-icon name="close" @click.stop.prevent="excelUpload = null" class="cursor-pointer" />
          </template>
      ...
      <q-card-actions class="justify-end">
        <q-btn @click="actividadesSubir = []; excelUpload = null" flat color="negative" v-close-popup>Cancelar</q-btn>
        <q-btn v-if="actividadesSubir.length == 0" @click="obtenerDistributivoGeneral" color="positive">Aceptar</q-btn>
...
const columns = [
  { name: 'sigla', align: 'center', label: 'CODIFICACIÓN', field: 'sigla', sortable: true },
...
async function obtenerDistributivoGeneral() {
  ...
  await readXlsxFile(file).then((rows) => {
    ...
    for (let i = 1; i < rows.length; i++) {
      if (i == 1 && rows[i][0] == null) return generateMessage('NO OK', 'El documento .xlsx no sigue el formato sugerido')
      if (rows[i][0] != null) {
