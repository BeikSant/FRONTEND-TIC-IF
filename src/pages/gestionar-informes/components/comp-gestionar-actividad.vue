<template>
  <q-dialog
    v-model="modalVerActividadEspecifica"
    persistent
    square
  ...
  >
    <q-card style="max-width: 700px">
      ...
      <q-tabs
        @vnode-before-update="estaEditando"
        indicator-color="white"
        v-model="tab_actividad"
        dense
        outside-arrows
        mobile-arrows
      ...
      >
        <q-tab
          :name="formatoInforme.actividadesDesarrolladas"
          :label="formatoInforme.actividadesDesarrolladas"
        />
        <q-tab :name="formatoInforme.evidencias" :label="formatoInforme.evidencias" />
        <q-tab
          :name="formatoInforme.observaciones"
          :label="formatoInforme.observaciones"
        />
      </q-tabs>
        ...
        <q-tab-panels v-model="tab_actividad">
          ...
          <q-tab-panel :name="formatoInforme.actividadesDesarrolladas">
            <q-list bordered>
              ...
              <q-expansion-item
                v-model="expansion_desarrolladas"
                dense
                group="somegroup"
                ...
                <q-item class="no-padding">
                  <q-item-section>
                    <q-editor
                      ...
                      flat
                      class="text-body2"
                      v-model="textActividadDesarrollada"
                      min-height="4rem"
                      ref="editorRef"
                      @paste="pegarEditor"
                      :definitions="definitionsEditor"
                      :toolbar="toolbarEditor"
                      toolbar-toggle-color="blue-7"
                      toolbar-bg="grey-5"
            ...
            <q-list v-else bordered separator dense>
              <template v-for="(item, key) in actividadesDesarrolladas" :key="key">
                ...
                <q-item
                  v-if="estaEditandoDesarrolladas && itemEditCampo == key"
                  class="no-padding"
                >
                  <q-item-section style="border: 1px solid rgba(0, 0, 0, 0.5)">
                    ...
                    <q-item-label class="no-margin">
                      <q-editor
                        class="text-body2 shadow-0"
                        square
                        flat
                        v-model="textEditarDesarrollada"
                        min-height="4rem"
                        ref="editorRef"
                        @paste="pegarEditor"
                        :definitions="definitionsEditor"
                  ...
                  <q-item-section side class="col-2">
                    <q-item-label>
                      <q-btn
                        ...
                        size="sm"
                        icon="mdi-file-edit"
                        @click="mostrarEditorEditar(item)"
                      >
          ...
          <q-tab-panel :name="formatoInforme.evidencias">
            <q-list bordered separator dense>
              <!-- Inicio Texto de entrada -->
              <q-expansion-item
                v-model="expansion_evidencias"
                dense
                group="somegroup"
                header-class="bg-grey-5"
                class="text-subtitle2"
                dense-toggle
                :label="'Agregar nuevas ' + formatoInforme.evidencias.toLowerCase()"
                icon="mdi-plus-box"
                :caption="expansion_evidencias ? '' : 'Expandir Aquí'"
              >
                <q-item class="no-padding">
                  <q-item-section>
                    <q-item-label>
            ...
            >
              <q-toolbar-title class="text-subtitle2 row justify-between text-bold">
                <span> Lista de {{ formatoInforme.evidencias.toLowerCase() }} </span>
            ...
            <q-list v-if="evidencias.length == 0" bordered separator dense>
              <q-item>
                ...
                <q-item-section class="text-weight-light"
                  >Aun no tiene
                  {{ formatoInforme.evidencias.toLowerCase() }} agregadas</q-item-section
                >
              </q-item>
            </q-list>
            ...
            <q-list v-else bordered separator dense>
              <template v-for="(item, key) in evidencias" :key="key">
                ...
                <q-item
                  v-if="estaEditandoEvidencias && itemEditCampo == key"
                  class="no-padding"
                >
                  <q-item-section style="border: 1px solid rgba(0, 0, 0, 0.5)">
                    <q-item-label class="bg-grey-5 q-px-md q-py-sm">
                      <div class="row justify-between">
                        <div class="text-body2">
                          <b
                            >Editar
                            {{
                              obtenerSingular(formatoInforme.evidencias.toLowerCase())
                            }}:</b
                          >
                        </div>
                        <div>
                ...
                <q-item v-else class="no-padding">
                  <q-item-section avatar class="col-1">
                    ...
                    <q-btn
                      ...
                      size="xs"
                      round
                      :disable="key == evidencias.length - 1"
                      @click="cambiarPosicionItem(item, key + 1 + 1)"
                    >
                      <q-tooltip class="bg-indigo"> Bajar </q-tooltip></q-btn
                    >
          ...
          <q-tab-panel :name="formatoInforme.observaciones">
            <q-list bordered separator dense>
              <!-- Inicio Texto de entrada -->
              <q-expansion-item
                v-model="expansion_observaciones"
                dense
                group="somegroup"
                header-class="bg-grey-5"
                class="text-subtitle2"
                dense-toggle
                :label="'Agregar nuevas ' + formatoInforme.observaciones.toLowerCase()"
                icon="mdi-plus-box"
            ...
            <q-toolbar
              class="bg-primary text-white q-mt-md q-py-xs"
              style="min-height: 35px"
            >
              <q-toolbar-title class="text-subtitle2 row justify-between text-bold">
                <span> Lista de {{ formatoInforme.observaciones.toLowerCase() }} </span>
            ...
            <q-list v-else bordered separator dense>
              <template v-for="(item, key) in observaciones" :key="key">
                ...
                <q-item
                  v-if="estaEditandoObservaciones && itemEditCampo == key"
                  class="no-padding"
                >
                  <q-item-section style="border: 1px solid rgba(0, 0, 0, 0.5)">
                    <q-item-label class="bg-grey-5 q-px-md q-py-sm">
                      <div class="row justify-between">
                        <div class="text-body2">
                          <b
                            >Editar
                            {{
                              obtenerSingular(formatoInforme.observaciones.toLowerCase())
                            }}:</b
                          >
                        </div>
                        <div>
    ...
    <q-card style="max-width: 400px">
      ...
      >
        ...
        <span
          ...
          <b>{{ obtenerSingular(tab_actividad.toLowerCase()) }}</b
          >?
        </span>
      </q-card-section>
...
const emits = defineEmits(["closeActividad"]);
const $q = useQuasar();
...
async function obtenerEvidencias() {
  await evidenciaController.obtenerEvidencias(props.actividad._id, async (res) => {
    ...
    if (res.status != 200)
      return generateMessage("NO OK", "Ocurrió un error al gestionar la actividad");
...
function guardar() {
  ...
  if (tab_actividad.value == props.formatoInforme.actividadesDesarrolladas)
    return guardarActividadDesarrollada();
  ...
  if (tab_actividad.value == props.formatoInforme.observaciones)
    return guardarObsevacion();
}

async function guardarEvidencia() {
  textEvidencia.value = sanitizarTexto(textEvidencia.value);
  if (textEvidencia.value == "")
    return generateMessage("NOOK", "No se puede guardar texto vacio");
  let data = {
  ...
  if (res.status != 200)
    return generateMessage(
      "NO OK",
      "Ocurrio un error al guardar la " + obtenerSingular(props.formatoInforme.evidencias)
    );
  textEvidencia.value = "";
...
async function guardarActividadDesarrollada() {
  ...
  if (textActividadDesarrollada.value == "")
    return generateMessage("NOOK", "No se puede guardar texto vacio");
  let data = {
    actividadEspecifica: props.actividad._id,
  ...
  if (res.status != 200)
    return generateMessage(
      "NO OK",
      "Ocurrio un error al guardar la " +
        obtenerSingular(props.formatoInforme.actividadesDesarrolladas)
    );
  textActividadDesarrollada.value = "";
  generateMessage(
    "OK",
    obtenerSingular(props.formatoInforme.actividadesDesarrolladas) + " guardada"
  );
...
async function guardarObsevacion() {
  ...
  if (res.status != 200)
    return generateMessage(
      "NO OK",
      "Ocurrio un error al guardar la " +
        obtenerSingular(props.formatoInforme.observaciones)
    );
  textObservacion.value = "";
  generateMessage(
    "OK",
    obtenerSingular(props.formatoInforme.observaciones) + " guardada"
  );
...
function borrarEditor() {
  if (tab_actividad.value == props.formatoInforme.actividadesDesarrolladas)
    return (textActividadDesarrollada.value = "");
  if (tab_actividad.value == props.formatoInforme.evidencias)
    return (textEvidencia.value = "");
  if (tab_actividad.value == props.formatoInforme.observaciones)
    return (textObservacion.value = "");
}

async function mostrarModalEliminar(item) {
  if (
    estaEditandoDesarrolladas.value ||
    estaEditandoEvidencias.value ||
    estaEditandoObservaciones.value
  ) {
    return generateMessage(
      "WARNING",
      "Aún se encuentra editando una " +
        obtenerSingular(tab_actividad.value.toLowerCase())
    );
  }
...
async function eliminarEvidencia() {
  await evidenciaController.eliminarEvidencia(item_eliminar.value._id, (res) => {
    if (res.status != 200)
      return generateMessage(
        "NO OK",
        "Ocurrió un error al eliminar la " +
          obtenerSingular(props.formatoInforme.evidencias.toLowerCase())
      );
    generateMessage(
      "OK",
      obtenerSingular(props.formatoInforme.evidencias) + " eliminada con éxito"
    );
  ...
  await desarrolladaController.eliminarDesarrollada(item_eliminar.value._id, (res) => {
    if (res.status != 200)
      return generateMessage(
        "NO OK",
        "Ocurrió un error al eliminar la " +
          obtenerSingular(props.formatoInforme.actividadesDesarrolladas.toLowerCase())
      );
    generateMessage(
      "OK",
      obtenerSingular(props.formatoInforme.actividadesDesarrolladas) +
  ...
  await observacionController.eliminarObservacion(item_eliminar.value._id, (res) => {
    if (res.status != 200)
      return generateMessage(
        "NO OK",
        "Ocurrió un error al eliminar la " +
          obtenerSingular(props.formatoInforme.observaciones.toLowerCase())
      );
    generateMessage(
      "OK",
      obtenerSingular(props.formatoInforme.observaciones) + " eliminada con éxito"
    );
    item_eliminar.value = null;
    modal_eliminar.value = false;
    obtenerObservaciones();
...
async function editarActividadDesarrollada() {
  ...
  await desarrolladaController.editarDesarrollada(
    desarrolladaEditar.value._id,
    { nombre: textEditarDesarrollada.value },
    async (res) => {
      if (res.status == 401) {
        generateMessage("NO OK", res.message);
        return router.push({ path: "/login" });
...
async function editarEvidencia() {
  ...
  await evidenciaController.editarEvidencia(
    evidenciaEditar.value._id,
    { nombre: textEditarEvidencia.value },
    async (res) => {
      if (res.status == 401) {
        generateMessage("NO OK", res.message);
        return router.push({ path: "/login" });
      ...
      generateMessage(
        "OK",
        obtenerSingular(props.formatoInforme.evidencias) + " editada con éxito"
      );
      cancelarEdicion();
      await obtenerEvidencias();
    }
...
async function editarObservacion() {
  ...
  await observacionController.editarObservacion(
    observacionEditar.value._id,
    { nombre: textEditarObservacion.value },
    async (res) => {
      if (res.status == 401) {
        generateMessage("NO OK", res.message);
        return router.push({ path: "/login" });
      ...
      generateMessage(
        "OK",
        obtenerSingular(props.formatoInforme.observaciones) + " editada con éxito"
      );
      cancelarEdicion();
      await obtenerObservaciones();
    }
